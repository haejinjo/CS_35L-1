...
int nthreads;
scene_t scene;
//created with the enums width and height (given)
float scaled_color[width][height][3];

void* draw(void* arg)//ptr will indicate which thread it is
{
	...
	/* for every pixel */
	for (int px = *threadnum; px < width; px += nthreads)//CHANGE THIS BASED ON THREADS
	{
		...
		for (int py = 0; py < height; ++py)
		{
			...
			*/
			...
			//CHANGED--since I want to load it into the buffer, I do it like this:
			//gamma gives a percentage, and I multiply by max_color to get my val
	...
	return NULL; //i don't need to do anything with the return value
}
...
main( int argc, char **argv )
{
	...
	//I want to use malloc in order to create some thread IDs
	...
	//I want to initialize my the argument I want to pass in
	int *row = (int*)malloc(nthreads*sizeof(int));
	if (row == NULL)
	{
		printf("Error allocating memory\n");
		exit(-1);
	...
	//I want to pass in this argument when creating the thread
	int i = 0;//initialize the iterator

	//initializing everything to zero
	for (i = 0; i < nthreads; i++)
		row[i] = i;//initialized to 0,1,2...nthreads

	//I call the multithreading function
	int created;
	for (i = 0; i < nthreads; i++)
	{
		created = pthread_create(tids + i, 0, draw, &row[i]);
		if (created)
		{
			printf("Error creating thread, returned error value %d\n", 
                               created);
			exit(-1);
		}
	}

	//I want to wait for threads to complete in order to prevent race condition
	//when dealing with printing scaled_color
	for (i = 0; i < nthreads; i++)
		pthread_join(tids[i], 0);
        //just in case...since you're printing the rows
        //that are already processed...

	//I want to print this pixel out to disk. This was copied from before
